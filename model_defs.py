...
def model_cnn_dm_small(in_ch, in_dim, width, linear_size=100): 
    model = nn.Sequential(
        nn.Conv2d(in_ch, 4*width, 4, stride=2, padding=2),
        nn.ReLU(),
...
def model_cnn_dm_medium(in_ch, in_dim, width, linear_size=512): 
    model = nn.Sequential(
        nn.Conv2d(in_ch, 4*width, 3, stride=1, padding=1),
        nn.ReLU(),
...
if __name__ == "__main__":
    ...
    in_dim= 28
    width = 4
    model = model_cnn_dm_small(in_ch, in_dim, width, linear_size=100)
    print(model)
    ...
    in_dim= 28
    width = 8
    model = model_cnn_dm_medium(in_ch, in_dim, width, linear_size=512)
    print(model)
    x = torch.rand(5,1,28,28)
    out = model(x)
    print(out.shape)
