...
def config_modelloader(config, load_pretrain = False, cuda = False):
    ...
    # config["model_def"] is the .py file that defines the model structure
    # os.path.splitext(config["model_def"])[0] is the filename without .py
    model_module = importlib.import_module(os.path.splitext(config["model_def"])[0])
    models = [] # models contains a list of models of different structures to be trained
    model_names = []
    # config is a dict
    # config["models"] is a list of dict, each dict specifies a model structure
    for model_config in config["models"]:
        if "ignore" in model_config and model_config["ignore"]:
            continue
        ...
        # model_config["model_class"] is the name of the class defined in the file model_module
        model_class = getattr(model_module, model_config["model_class"])
        # model_config["model_params"] is a dict that contains the keywords and their values
        # to the constructor of the class model_class
        model_params = model_config["model_params"]
        m = model_class(**model_params) # m is a constructed neural network
        if cuda:
            m.cuda()
        if load_pretrain:
